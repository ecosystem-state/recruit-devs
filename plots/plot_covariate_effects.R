...
out_df = readRDS("output/summaries/parameteric_univariate.rds")
out_ml_df = readRDS("output/summaries/nonparameteric_univariate_raw.rds")

#
out <- dplyr::filter(out_df, years_ahead == 1, n == 10) %>%
...
for(i in 1:nrow(out0)) {
  sub <- dplyr::filter(out_df, years_ahead == 0, n == 10, id == out0$id[i],
                model == out0$model[i], spp == out0$spp[i],
                predictors == out0$predictors[i])
  out0$rmse_train[i] <- sub$rmse
  out0$r2_train[i] <- sub$r2
}
...
lasso_models = dplyr::filter(out_ml_df,
                  years_ahead == 1,
                  alpha == 1,
                  lambda <= 2,
                  model == "glmnet") %>%
...
rf_models = dplyr::filter(out_ml_df,
                             years_ahead == 1,
                             model == "randomForest") %>%
  dplyr::group_by(spp, predictors) %>%
...
ml0$r2_train <- 0
for(i in 1:nrow(ml0)) {
  sub <- dplyr::filter(out_ml_df, years_ahead == 0, n == 10, id == ml0$id[i],
                       model == ml0$model[i], spp == ml0$spp[i],
                       predictors == ml0$predictors[i])
  ml0$rmse_train[i] <- sub$rmse
  ml0$r2_train[i] <- sub$r2
}
test_train <- rbind(out0, ml0)

test_train <- dplyr::rename(test_train, Model = model)
p11 <- ggplot(test_train, aes(r2_train, r2, col=Model)) +
  geom_point(alpha=0.6) +
  theme_bw() +
  xlab("Train") + ylab("Test") +
  scale_color_viridis(discrete = TRUE, end=0.8)
ggsave("plots/Figure_S11.png", plot = p11, width=7, height=7)
...
p1 <- ggplot(out, aes(x = predictors, y = rmse, col=Model)) +
...
ggsave("plots/Figure_2.png", plot = p1, width=7, height=7)

p2 <- ggplot(out, aes(x = predictors, y = r2, col=Model)) +
  geom_beeswarm(alpha=0.6, dodge.width = 1) +
  xlab("") +
  ylab("R-squared") +
  scale_color_viridis(discrete = TRUE, end=0.8) +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5))
ggsave("plots/Figure_S4.png", plot = p2, width=7, height=7)
...
ggsave("plots/Figure_S5.png", plot = p1_by_species, width=7, height=7)
...
p2_by_species <- ggplot(out, aes(x = predictors, y = r2, col=Model)) +
  ...
  theme(strip.background=element_rect(fill="white"),
        strip.text.x = element_text(size = 6.5),
        axis.text.x = element_text(size=7))
ggsave("plots/Figure_S6.png", plot = p2_by_species, width=7, height=7)
