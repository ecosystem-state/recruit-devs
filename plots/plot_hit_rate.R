...
#ggsave("plots/Figure_2.pdf", plot = p1, width=7, height=7)
...
# false rate_lo and rate_hi are the false alarm rates ()
# false_rate_lo is the error we most want to avoid (rec dev is lo, and we don't detect it)
# false_rate_hi is the opposite
out_long <- tidyr::pivot_longer(out, cols = c("false_rate_lo", "false_rate_hi"))
out_long <- dplyr::rename(out_long, Truth = name)
out_long$Truth[which(out_long$Truth == "false_rate_lo")] = "Lower tercile"
out_long$Truth[which(out_long$Truth == "false_rate_hi")] = "Upper tercile"

false_rate_by_species <- ggplot(out_long, aes(x = predictors, y = 1 - value, col=Truth)) +
  geom_point(alpha=0.6, aes(shape = Model)) +
  xlab("") +
  ylab("Frequency of forecasting correctly") +
  #scale_color_viridis(discrete = TRUE, end=0.8) +
  scale_color_viridis(option="A", discrete = TRUE, begin=0.4, end=0.8) +
  theme_bw() +
  coord_cartesian(ylim=c(0.0,0.9)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5)) +
  facet_wrap(~Species, scale="free_y") +
...
ggsave("plots/Figure_S8.png", plot = false_rate_by_species, width=7, height=7)
